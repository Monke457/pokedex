...
func ExploreLocation(loc string) ([]string, error) {
	...
	jsonEnc := Encounters{}
	err = json.Unmarshal(res, &jsonEnc)
	if err != nil {
		return nil, err
	}

	pokemon := []string{}
	for _, p := range jsonEnc.PokemonEncounters {
...
func GetPokemon(name string) (*pokedex.Pokemon, error) {
